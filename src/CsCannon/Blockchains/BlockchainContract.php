...
abstract class  BlockchainContract extends Entity implements Displayable
{
    ...
    public function getCollections(bool $loadDefault = false): ?array
    {
        ...
        if (!is_array($collectionEntities) && $loadDefault) {
            $assetCollectionFactory = new AssetCollectionFactory(SandraManager::getSandra());
            $assetCollectionFactory->populateFromSearchResults("default");
            $assetCollectionFactory->getTriplets();
            return $assetCollectionFactory->getEntities();
        }
